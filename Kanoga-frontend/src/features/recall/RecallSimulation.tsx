  ...
  async function handleSimulate() {
    ...
    try {
      ...
      const data = await apiGet<RecallResultDto>(
        `/api/recall?batchCode=${encodeURIComponent(trimmed)}`,
        auth
      );
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <p style={{ fontSize: 13, color: "#4b5563", marginBottom: 14 }}>
        Enter a batch code to trace affected sub-batches, units, orders, and customers.
      </p>
      ...
      {result && (
        <>
          ...
          >
            ...
            <StatCard
              label="Affected sub-batches"
              value={String(result.affectedSubBatches)}
            />
            <StatCard label="Affected units" value={String(result.affectedUnits)} />
            <StatCard
              label="Affected orders"
              value={String(result.affectedOrders)}
            />
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <StatCard
              ...
              value={String(result.affectedCustomers)}
            />
          </div>
          ...
          >
            ...
            {result.units.length === 0 ? (
              <div style={{ fontSize: 13, color: "#6b7280" }}>
                No matching units found for this batch code.
              </div>
            ) : (
                ...
                >
                  ...
                  <tbody>
                    {result.units.map((unit, index) => (
                      <tr key={`${unit.labelId ?? "no-label"}-${index}`}>
                        <td style={tdStyle}>{unit.subBatchCode ?? "—"}</td>
                        <td style={tdStyle}>{unit.labelId ?? "—"}</td>
                        <td style={tdStyle}>{unit.serialNo ?? "—"}</td>
                        <td style={tdStyle}>{unit.orderId ?? "—"}</td>
                        <td style={tdStyle}>{unit.orderNumber ?? "—"}</td>
                        <td style={tdStyle}>{unit.customerName ?? "—"}</td>
                        <td style={tdStyle}>{unit.customerEmail ?? "—"}</td>
                        <td style={tdStyle}>{unit.assignedAt ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
